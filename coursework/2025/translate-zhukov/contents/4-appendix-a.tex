...
\begin{algorithm}[htp!]
    ...
    \KwData{базис решётки $\text{b}_1, \dots, \text{b}_n \in \mathbb{Z}^m$, параметр $\delta$}
...
\subsection*{Алгоритм ближайшей плоскости Бабаи}
...
\begin{algorithm}[htp!]
    ...
    \KwData{базис решётки $\text{b}_1, \dots, \text{b}_n \in \mathbb{Z}^m$, параметр $\delta = 3/4$, целевой вектор $\text{t} \in \mathbb{Z}^m$}
